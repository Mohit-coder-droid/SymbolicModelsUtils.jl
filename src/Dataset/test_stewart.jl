...
problems = [
    ...
    [log(x), x, 1, -x + x * log(x)],
    [exp(x) * x^2, x, 3, 2 * exp(x) - 2 * exp(x) * x + exp(x) * x^2],  # solved
    [exp(x) * sin(x), x, 1, -1 // 2 * exp(x) * cos(x) + 1 // 2 * exp(x) * sin(x)],  # to be solved later
    [atan(x), x, 2, x * atan(x) - 1 // 2 * log(1 + x^2)],
    [exp(2 * x) * x, x, 2, -1 // 4 * exp(2 * x) + 1 // 2 * exp(2 * x) * x], # solved
    ...
    [cos(3 * t) / exp(t), t, 1, -1 // 10 * cos(3 * t) / exp(t) + 3 // 10 * sin(3 * t) / exp(t)],   # to be solved later 
    ...
    [1 / (x^2 * sqrt(4 + x^2)), x, 1, -1 // 4 * sqrt(4 + x^2) / x, SI.NotImplementedError],  # solved
    [x / sqrt(4 + x^2), x, 1, sqrt(4 + x^2), SI.NotImplementedError],
    [1 / sqrt(-a^2 + x^2), x, 2, atanh(x / sqrt(-a^2 + x^2)), SI.NotImplementedError],    # try solving this
    [x^3 / (9 + 4 * x^2)^(3 // 2), x, 3, 9 // 16 / sqrt(9 + 4 * x^2) + 1 // 16 * sqrt(9 + 4 * x^2), SI.NotImplementedError],# exponent 3//2
    [x / sqrt(3 - 2 * x - x^2), x, 3, asin(1 // 2 * (-1 - x)) - sqrt(3 - 2 * x - x^2), SI.NotImplementedError],   # solved
    ...
    [sqrt(1 + x^2), x, 2, 1 // 2 * asinh(x) + 1 // 2 * x * sqrt(1 + x^2), SI.NotImplementedError],   # solve this
    [1 / (x^3 * sqrt(-16 + x^2)), x, 4, 1 // 128 * atan(1 // 4 * sqrt(-16 + x^2)) + 1 // 32 * sqrt(-16 + x^2) / x^2, SI.NotImplementedError],
    [sqrt(-a^2 + x^2) / x^4, x, 1, 1 // 3 * (-a^2 + x^2)^(3 // 2) / (a^2 * x^3), SI.NotImplementedError],# sqrt, a    # secx type question
    ...
    [x / (4 + x^2)^(5 // 2), x, 1, (-1 // 3) // (4 + x^2)^(3 // 2), SI.NotImplementedError],# exponent 5//2    # if not much is found then just take a question where we have to substitute sec(x) 
    ...
    [5 * x * sqrt(1 + x^2), x, 2, 5 // 3 * (1 + x^2)^(3 // 2), SI.NotImplementedError],   # solved 
    ...
    [sqrt(-9 + exp(2 * t)), t, 4, -3 * atan(1 // 3 * sqrt(-9 + exp(2 * t))) + sqrt(-9 + exp(2 * t)), SI.NotImplementedError],   # solved
    ...
    [(-1 + 2 * x + x^2) // (-2 * x + 3 * x^2 + 2 * x^3), x, 3, 1 // 10 * log(1 - 2 * x) + 1 // 2 * log(x) - 1 // 10 * log(2 + x)],  # triple linear partial fraction (PF)
    [(1 + 4 * x - 2 * x^2 + x^4) // (1 - x - x^2 + x^3), x, 2, 2 // (1 - x) + x + 1 // 2 * x^2 + log(1 - x) - log(1 + x)],  # two PF, 1 quadratic
    [(4 - x + 2 * x^2) // (4 * x + x^3), x, 6, -1 // 2 * atan(1 // 2 * x) + log(x) + 1 // 2 * log(4 + x^2)],  # polynomiall division, two PF, 1 quad
    [(2 - 3 * x + 4 * x^2) // (3 - 4 * x + 4 * x^2), x, 6, x + 1 // 8 * log(3 - 4 * x + 4 * x^2) + 1 // 4 * atan((1 - 2 * x) / sqrt(2)) / sqrt(2)], # too much lonh problem
    [(1 + x^2 + x^3) // ((-1 + x) * x * (1 + x^2)^3 * (1 + x + x^2)), x, 14, 1 // 8 * (1 + x) // (1 + x^2)^2 - 3 // 8 * (1 - x) // (1 + x^2) + 3 // 16 * x // (1 + x^2) + 7 // 16 * atan(x) + 1 // 8 * log(1 - x) - log(x) + 15 // 16 * log(1 + x^2) - 1 // 2 * log(1 + x + x^2) - atan((1 + 2 * x) / sqrt(3)) / sqrt(3)],  # too much lonh problem
    [(1 - 3 * x + 2 * x^2 - x^3) // (x * (1 + x^2)^2), x, 6, 1 // 2 * (-1 - 2 * x) // (1 + x^2) - 2 * atan(x) + log(x) - 1 // 2 * log(1 + x^2)],  # too long problem
    ...
    # one way to do above problem is to do partial fraction, which is going to be too lengthy 
    ...
    [sec(x) * tan(x)^2, x, 2, -1 // 2 * atanh(sin(x)) + 1 // 2 * sec(x) * tan(x)],  # try solving this 
    [sec(x)^3 * tan(x)^3, x, 3, -1 // 3 * sec(x)^3 + 1 // 5 * sec(x)^5],
    [exp(sqrt(x)), x, 3, -2 * exp(sqrt(x)) + 2 * exp(sqrt(x)) * sqrt(x), SI.NotImplementedError],
    ...
    [x // (2 - 2 * x + x^2), x, 4, -atan(1 - x) + 1 // 2 * log(2 - 2 * x + x^2)],  # solve this (easy) (partial fraction not applicable)
    ...
    [x^2 * log(1 + x), x, 3, -1 // 3 * x + 1 // 6 * x^2 - 1 // 9 * x^3 + 1 // 3 * log(1 + x) + 1 // 3 * x^3 * log(1 + x)],  # by parts + PF
    ...
    [1 // (4 - 5 * sin(x)), x, 4, -1 // 3 * log(cos(1 // 2 * x) - 2 * sin(1 // 2 * x)) + 1 // 3 * log(2 * cos(1 // 2 * x) - sin(1 // 2 * x))],  # try solving this (I think I hadn't included derivatives of inverse trig func, which will be helpful while making some substituion)
    ...
    [sin(x) * sin(2 * x) * sin(3 * x), x, 5, -1 // 8 * cos(2 * x) - 1 // 16 * cos(4 * x) + 1 // 24 * cos(6 * x)],  # solve this
    ...
    [1 // (sqrt(x) + sqrt(1 + x)), x, 3, -2 // 3 * x^(3 // 2) + 2 // 3 * (1 + x)^(3 // 2), SI.NotImplementedError],  # solve this (Rationalization)
    ...
    [log(1 + x) // x^2, x, 4, log(x) - log(1 + x) - log(1 + x) // x],  # by parts + PF
    ...
    [cos(x) * sin(x) // sqrt(1 + sin(x)), x, 3, 2 // 3 * (1 + sin(x))^(3 // 2) - 2 * sqrt(1 + sin(x)), SI.NotImplementedError],   # solve this
    ...
    [x^5 // (x^2 + sqrt(2)), x, 3, 1 // 4 * x^4 + log(x^2 + sqrt(2)) - x^2 / sqrt(2), SI.NotImplementedError],
    [sec(x)^5, x, 3, 3 // 8 * atanh(sin(x)) + 3 // 8 * sec(x) * tan(x) + 1 // 4 * sec(x)^3 * tan(x)],   # solve this (Standard integral reduction formula)
    ...
    [x^2*sqrt(5-x^2), x, 3, 25 // 8 * asin(x / sqrt(5)) - 5 // 8 * x * sqrt(5 - x^2) + 1 // 4 * x^3 * sqrt(5 - x^2), SI.NotImplementedError],  # both chatgpt and deep-ai/maths got this wrong, (claude, copilot, deepseek) got it right 
